...
import { GripVertical, Trash2, ArrowUp, ArrowDown, Copy, PlusCircle } from 'lucide-react';
...
const BlockSettingsEditor = ({ block }: { block: Block }) => {
    const { 
        updateBlockContent,
        addQuizOption,
        updateQuizOption,
        ...
        <div className="relative group" onClick={() => setActiveBlockId(block.id)}>
            <Card className={cn("transition-all mb-4", isActive ? 'border-primary ring-2 ring-primary/50' : 'hover:border-primary/50')}>
                <CardContent className="p-4">
                    {isActive ? <BlockSettingsEditor block={block} /> : <BlockRenderer block={block} />}
                </CardContent>
            </Card>
